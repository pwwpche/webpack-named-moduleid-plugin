...
function replaceModuleId(webpackModule, modulePrefix, hideDependencies) {
  var resourceLocation = webpackModule.resource;
  var replacedId = '';
  ...
  if (resourceLocation && resourceLocation.indexOf('node_modules/') !== -1) {
    ...
    // module.id
    replacedId = webpackModule.id.toString();
  }

  if (webpackModule.id.toString().startsWith(modulePrefix)) {
    // The module id is already processed, skip adding prefix to it.
    return replacedId;
  } else {
    return modulePrefix + replacedId;
  }
}
    ...
    compilation.plugin('after-optimize-chunk-ids', (chunks) => {
      ...
      chunks.forEach((chunk) => {
        ...
        chunk.forEachModule((module) => {
          module.id = replaceModuleId(module, prefix, hideDependencies);
        });
      });
    });
  });
